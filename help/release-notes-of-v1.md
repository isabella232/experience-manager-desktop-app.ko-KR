...
>[!NOTE]
>
>캐시 크기 제한이 적용되지 않습니다. 데스크탑 앱이 시작되면 캐시 크기가 한 번 계산되며 크기가 사전 정의된 제한에 근접하면 알림이 표시됩니다.
...
Adobe은 최신 기능, 최신 버그 수정 및 최상의 성능을 이용할 수 있도록 최신 버전의 AEM 데스크탑 앱을 사용하는 것이 좋습니다.

로컬 컴퓨터에 설치하려는 AEM 데스크탑 앱의 버전은 특정 AEM 서버 버전/서버측 추가 구성 요소(서비스 팩, 핫픽스 또는 기능 팩)가 필요합니다. 처음으로 연결하기 전에 AEM 서버가 제대로 구성되어 있는지 확인합니다. 도움이 필요한 경우 AEM 관리자에게 문의하십시오.
...
**업데이트 충돌에 대한 알림입니다**. 자산을 업데이트하려고 할 때 애플리케이션에서 충돌을 감지하면 알림이 표시되므로 사용자가 상태 창을 모니터링하지 않고도 이를 검토할 수 있습니다. 응용 프로그램이 시작되면 사용자가 해결할 수 있도록 모든 충돌을 확인합니다.

**연결 손실의 처리** 개선. 연결 손실이 있으면 벌크 업로드는 일시 중지되며 사용자는 나중에 다시 시작할 수 있습니다. 개별 파일의 실패한 업로드를 다시 시도하는 데 [!UICONTROL Retry] 옵션을 사용할 수 있습니다.
...
AEM 데스크탑 앱 1.9는 연결된 파일(예: InDesign)이 있는 자산을 열 때 대규모 업로드에 대한 사용자 경험, 백그라운드 작업에 대한 정보 및 최적화된 경험을 개선하는 데 중점을 두었습니다.

**다시 시작**
가능한 업로드특히 큰 파일 주변에서 업로드하려면 새 자산 상태 창에서 이 파일을 일시 중지/다시 시작하는 옵션이 있습니다.
...
* 진행률 표시줄, 전송 속도, 총 파일 크기 및 지금까지 전송된 크기를 포함하여 진행 중인 업로드를 표시합니다.

* 완료된 업로드가 총 전송 및 최종 전송 속도로 표시됩니다.

* 사용 가능한 경우 오류 메시지 및 전송 정보와 함께 업로드에 실패한 정보가 표시됩니다.

* 3번 실패한 업로드하면 오류 메시지가 표시됩니다.

* 사용자가 클릭할 수 있는 아이콘과 함께 표시되는 충돌하는 파일입니다. 아이콘을 클릭하면 다음과 같은 두 가지 옵션이 있는 대화 상자가 표시됩니다.
...
* 완료된 다운로드에는 총 전송, 최종 비율 및 클릭하면 파일이 열립니다(단일 파일만 사용 가능).

* 오류 메시지 및 전송 정보가 표시된 다운로드 실패(가능한 경우)

* 바닥글은 다운로드한 총 파일 수와 평균 전송 속도를 보여줍니다.

* 사용자가 [!DNL Experience Manager Assets] 웹 인터페이스에서 여러 파일을 열거나 편집하도록 선택하면 함께 그룹화됩니다. 예를 들어 myasset.jpeg 및 4개 이상의 파일이 있습니다.

* AEM Assets에 저장된 연결된 자산을 포함하는 InDesign 문서를 다운로드할 때 데스크탑 앱은 먼저 연결된 모든 자산을 다운로드한 후 [!UICONTROL Adobe InDesign] 문서를 열고 연결된 자산의 다운로드를 표시합니다. 예를 들어 24개 중 5개.

[!UICONTROL Bulk Uploads]

AEM Web UI에서 [!UICONTROL Create] [!UICONTROL Upload Folder]을 통해 큰 폴더 계층을 업로드하거나 데스크탑 앱 컨텍스트 메뉴에서 Finder 또는 Explorer에서 &quot;자산 붙여넣기&quot;를 선택하고 복사하면 이 대화 상자의 사용이 트리거됩니다.

* 진행률 표시줄 및 현재 전송 중인 파일의 이름을 포함하여 진행 중인 업로드를 표시합니다.

* 진행 중인 업로드에는 클릭 시 업로드를 취소할 아이콘이 포함되어 있습니다. 현재 파일 전송이 완료되면 전송이 중지됩니다.

* 실패한 전송 프로세스가 오류 메시지와 함께 표시됩니다(전체 전송이 실패한 경우에만).

* 개별 파일을 전송하지 못하면 탭에 오류로 표시됩니다. 그렇지 않으면 개별 파일이 탭에 표시되지 않고 전체 업로드에 대한 단일 항목만 표시됩니다.

**백그라운드 작업의 상태를 나타내는 아이콘**

애플리케이션 아이콘은 사용자에게 더 나은 시각적 큐를 제공하기 위해 백그라운드 작업의 상태를 나타냅니다. 예를 들어 애플리케이션이 AEM에 연결되어 있지 않으면 아이콘이 회색으로 표시되며 활성 업로드가 있는 경우 &quot;동기화&quot; 오버레이 등이 표시됩니다.
...
* 캐싱: 다음을 포함한 AEM 데스크탑 앱 캐시 관리를 위한 새로운 UI(CQ-4208690)
   ...
   * 알림을 보내기 전 최대 캐시 크기 정의

   * 캐시 크기는 데스크탑 앱 시작에서만 확인되며 구성된 제한에 도달하는 경우 알림이 표시됩니다
...
* 네트워크:
   ...
   * AEM 세션이 만료되면 이제 사용자에게 알림을 보내고 알림을 클릭하여 다시 로그인할 수 있습니다(CQ-4202028).

   * (Mac) .pac 프록시 구성을 사용하여 AEM에 연결할 수 있는 지원을 추가합니다(CQ-4233430).

   * (Win) 고급 - 로그인 URL 대화 상자의 문제를 수정합니다(CQ-4236061).
...
* 안정성:
   ...
   * AEM 데스크탑 앱이 오버로드된 AEM 서버에 연결할 때 안정성을 개선했습니다(CQ-4224803).
...
* 설치 및 제거:
   ...
   * [Windows 32] bitInstaller에서 Microsoft .NET Framework v. 4.5를 설치하지 못했습니다(CQ-4218084).
...
* Finder/Explorer에서 네트워크 공유에 대한 캐싱 및 액세스:
   ...
   * 업로드 큐 상태에 보고된 충돌로 인해 파일 업로드 시 오류가 발생했습니다(CQ-4215727).

   * PS의 데스크탑 앱 폴더에서 여러 파일을 여는 동안 파일이 잘리거나 완료되지 않은 메시지가 표시될 수 있습니다(CQ-4216276).
...
* 안정성 및 성능 수정 사항:
   ...
   * 큐 상태 표시 기능은 앱을 설치한 사용자만 작동합니다(CQ-4212199).

   * (Windows) 32비트 설치 프로그램에 64비트 코드가 포함되어 있지 않은지 확인합니다(CQ-4217406).

* 1.6 베타에서 발견된 문제 및 해결된 문제:

   * 높은 CPU 사용(CQ-4218070).

   * AEM에 업로드할 때 파일 끌어다 놓기 오류가 발생합니다(CQ-4217006).
...
* 버그 수정:
   ...
   * 자산 이름 변경 중에 가끔 발생하는 오류 43 문제가 해결되었습니다(CQ-4207900).

   * AEM의 타임라인에서 이전 버전으로 되돌리면 Finder에서 자산이 업데이트되지 않습니다(CQ-4205194).

   * 큰 중첩된 디렉토리를 검색할 때 데스크탑 앱이 충돌합니다(CQ-4208539).
...
이 동작은 다음과 같은 예외를 제외하고 로컬 디스크의 자산 및 AEM Desktop 마운트 디렉토리의 자산에 대해 동일합니다.

* 배치된 자산이 다른 사용자에 의해 수정된 경우, 다른 사용자가 배치된 자산이 들어 있는 문서를 처음 열 때 경고 아이콘이 표시됩니다. 이 문제는 배치된 자산이 이미 로컬로 캐시된 경우에만 발생합니다.

* 사용자가 AEM 데스크탑의 마운트된 디렉토리를 통해 배치된 자산을 수정한 다음 로컬 캐시를 지우면 배치된 자산이 최신 상태로 보고됩니다.

이 두 경우 모두 예상되며 AEM Desktop의 &quot;지연된 동기화&quot; 아키텍처의 부작용입니다.

**Mac OS X 및 Windows용 버전 1.5.0.x:** AEM 데스크탑 앱의 이번 릴리스는 다음과 같은 이점을 제공합니다.

* 네트워킹 문제에 대한 안정성과 탄력성이 향상됩니다.

   * AEM Assets 폴더의 보다 안정적인 매핑(CQ-103276, CQ-4204669, CQ-4203957).

   * 캐시된 파일의 처리 개선(CQ-4204336, CQ-4206263).

   * 크기가 2GB 이상인 대용량 파일의 다운로드/업로드 처리를 개선했습니다(CQ-4206438).
...
* 기타 수정 사항:
   ...
   * SSL을 통한 로그인 처리 개선(CQ-4200217).

   * 더 안정적인 공유 마운팅(CQ-4200793).
...
* Touch UI에서 새로운 만들기 > 폴더 업로드 작업을 통해 계층 폴더의 간소화된 업로드.
   ...
   * 데스크탑 앱은 데스크탑의 지정된 폴더 계층 구조를 백그라운드로 전환하고 파일을 AEM Assets에 업로드합니다
   * 진행 중인 작업에 대한 진행 표시줄이 있는 새 업로드 대기열 상태 창의 진행 상태를 모니터링할 수 있습니다
   * 업로드 큐 상태도 더 나은 문제 해결 정보(예: 서버에 대한 연결 없음)를 제공합니다
* 체크 아웃 및 열기 작업을 하나로 결합하는 Touch UI의 새로운 편집 작업입니다
* Touch UI에서 데스크톱 관련 작업 그룹화가 최적화되었습니다(AEM 6.3)
...
* 캐싱 및 성능 개선.
* 데스크탑 앱(PS, ID, AI 등)에서 바로 저장할 수 있는 지원이 향상되었습니다.
...
로컬 시스템에 관리자 권한이 없는 사용자에 대해 AEM 데스크탑 앱을 실행해야 하므로 AEM Assets 저장소의 마운트 지점이 MacOS에서 사용자의 로컬 폴더에 있는 DAM 하위 폴더로 1.4 및 1.5에서 변경되었습니다(CQ-104183).

/Volumes 폴더에 더 이상 관리 권한이 필요하지 않으므로 이 변경 사항은 1.5.1로 되돌렸습니다. 따라서 MacOS 사용자 간에 AEM 자산을 배치한 InDesign 문서를 공유할 수도 있습니다.
...
>[!NOTE]
>
>Mac OS X에서 로컬 네트워크 드라이브 프로토콜을 WebDAV에서 SMB1로 변경하면 리포지토리의 동일한 자산에 다른 로컬 경로가 표시됩니다. 이 작업은 &quot;배치&quot; 명령을 통해 Adobe Creative Cloud 애플리케이션에 배치된 파일에 대한 링크에 영향을 줄 수 있습니다. 자세한 내용은 [AEM Desktop App](use-app-v1.md) 사용 을 참조하십시오.

### 파일 처리(1.3 이후) {#file-handling-since}

* 폴더는 사전 정의된 지연(현재 30초) 후에 자동으로 업데이트됩니다.
* 다른 사용자가 체크 아웃한 파일은 읽기 전용으로 표시됩니다.
* 파일은 데스크탑 앱을 통해 마운트된 네트워크 드라이브 위치에 두 단계로 저장됩니다.
* 첫 번째 단계에서는 파일이 로컬로 저장됩니다. 이 방법으로 파일을 저장하는 사용자는 파일이 AEM으로 완전히 전송될 때까지 기다리지 않아도 되며 파일이 저장되면 바로 작업을 재개할 수 있습니다.
* 두 번째 단계에서 데스크탑 앱은 사전 정의된 지연(예: 30s ) 후에 업데이트된 파일을 AEM 서버에 업로드합니다. 이 작업은 백그라운드에서 수행됩니다. 업로드 작업의 상태를 보려면 **배경 파일 동기화 상태 표시** 옵션을 사용합니다.

## 중요 공지 {#important-notices}

**폴더 업로드.** Finder/Explorer 수준에서 마운트된 AEM 리포지토리에 복사/드래그 앤 드롭을 사용하는 것과 대조적으로, 새로운 폴더 업로드 기능을 사용하여 크고 계층적인 폴더를 AEM에 업로드하는 것이 좋습니다. 폴더 업로드 기능을 사용할 때 데스크탑 앱은 AEM과 직접 통신하므로 전체 프로세스를 훨씬 잘 제어할 수 있습니다.

**AEM 세션을 사용 가능하게 유지합니다.** AEM 데스크탑 앱은 AEM Assets 서버에 열려 있는 세션에 따라 적절한 작업이 수행되는지 확인합니다. 매일 데스크탑 앱을 사용하는 사용자의 경우 로그아웃을 강제로 수행하려면 하루의 끝에 AEM Assets을 마운트 해제한 다음 아침에 &quot;AEM Assets 마운트&quot;를 수행하여 로그인 및 네트워크 공유가 작동 중인지 확인하는 것이 좋습니다.

**Finder에서 &quot;아이콘 미리 보기&quot;를 해제합니다.** Finder를 사용하는 대형 폴더의 성능 탐색을 위해 특히 네트워크 연결 속도가 느린 경우 &quot;아이콘&quot; 및 &quot;아이콘 미리 보기&quot;가 모두 꺼져 있는지 확인합니다. 그렇지 않으면 Finder가 폴더의 각 자산을 다운로드하여 작은 미리 보기를 생성하므로 성능이 저하되고 높은 대역폭 사용률이 발생합니다(CQ-4219779)
...
**새 AEM 서버에 연결할 때 캐시를 지웁니다.** 데스크탑 앱이 동일한 URL을 사용하는 다른 AEM 서버에 연결하는 경우 캐시가 자동으로 지워지지 않습니다. 캐시를 수동으로 지우면 적절한 작업이 보장됩니다. 이 문제는 일반적으로 동일한 URL에서 실행하는 동안 AEM 설치를 대체할 수 있는 경우 테스트에서 발생합니다(CQ-4216982)
...
* 파일 이름 / 로컬라이제이션:
   ...
   * 국제 이름이 있는 자산은 몇 초마다 표시되고 사라질 수 있습니다
* 체크인 및 체크아웃:
   * 한 사용자가 체크 아웃한 자산은 Touch UI에서 열기 작업으로 또는 데스크탑에서 직접 다른 사용자에 대해 열 수 없습니다. 일부 응용 프로그램은 잠긴것으로 보고될 수 있지만 열기를 시도하는 동안 손상되거나 정지될 수도 있습니다. (CQ-4199234)
   * 여러 사용자가 동시에 파일을 변경하면 일부 수정 사항이 손실될 수 있습니다. 해결 방법은 여러 사용자가 동일한 파일을 변경하지 못하도록 체크인 및 체크아웃 기능을 사용하는 것입니다(CQ-97035)
   * 특정 응용 프로그램은 사용자가 다른 사용자가 체크 아웃한 파일을 저장할 수 있도록 하는 읽기 전용 플래그를 제대로 지원하지 않습니다. 수정된 파일은 다른 사용자가 파일을 체크 인할 때까지 전송되지 않습니다. 두 수정 사항 모두 AEM에서 자산의 다른 버전으로 사용할 수 있습니다(CQ-89551, CQ-87572, CQ-89615)
...
* Windows:
   ...
   * 연결이 끊기면 일괄 업로드를 다시 시작한 후 특정 상황에서 실패한 업로드에서 [!UICONTROL Retry] 옵션을 선택하면 앱이 충돌할 수 있습니다(CQ-4251884)(Win)
